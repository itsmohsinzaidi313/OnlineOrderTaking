    ...
    internal class RequestQueueListener(ILogger<RequestQueueListener> logger, RabbitMqConnection rabbitConnection, Implementation impl, IRabbitMqPublisher publisher) : RabbitMqConsumerService<RequestQueueListener>(logger, rabbitConnection)
    {
        ...
        public override async Task OnMessage(string transport)
        {
            ...
            try
            {
                ...
                payload = await GetMenuItemsAsync(requestPayload.DomainName, requestPayload.BranchId).ToListAsync();
                success = true;
            }
            catch (Exception ex)
            {
        ...
        private async IAsyncEnumerable<Category> GetMenuItemsAsync(string url, int branchId)
        {
            logger.LogInformation("📂 Fetching menu items from database...");

            await foreach (var element in impl.GetMenuAsync(url, branchId))
            {
