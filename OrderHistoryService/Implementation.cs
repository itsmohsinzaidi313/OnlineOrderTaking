...
public class Implementation()
{
    ...
    internal async IAsyncEnumerable<CustomerOrder> GetOrdersAsync(string connectionString, int? userId = null, string? orderToken = null)
    {
        ...
        if (userId.HasValue)
        {
            ...
            var orderMasters = await dbContext.OrderMasters.Where(x => branchIds.Contains(x.BranchId) && x.OrderDate > DateOnly.FromDateTime(DateTime.Now.AddDays(-10))).ToListAsync();
        ...
        {
            var orderMaster = await dbContext.OrderMasters.FirstOrDefaultAsync(x => x.OrderToken == orderToken && x.OrderDate > DateOnly.FromDateTime(DateTime.Now.AddDays(-10)));
            if (orderMaster != null)
            {
